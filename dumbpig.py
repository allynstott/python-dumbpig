...
class RuleChecker(object):
    ...
    def __init__(self, dumbpig_search_path=("/usr/bin/dumbpig.pl",
        ...
        regex_version = re.compile(r"DumbPig version ([0-9]*\.[0-9]*[^ ])* \-")
        ...
        for dumbpig_path in dumbpig_search_path:
            ...
            raise RuleChecker.RuleCheckerError(
                "Dumbpig program was not found. Path: \"%s\"" %
                ", ".join(dumbpig_search_path))

        dp_out = proc_out.stdout.read()
        ...
        if not is_dumbpig_found:
            raise RuleChecker.RuleCheckerError("Dumbpig was not found. "
                                               "Please install dumbpig first")
    ...
    def set_rule_file(self, rule_path):
        ...
        if not os.path.isfile(rule_path):
            raise RuleChecker.RuleCheckerError("Rule file is not found. "
    ...
    def set_sensitivity(self, sensitivity):
        ...
        if (sensitivity < 1) or (sensitivity > 4):
            raise RuleChecker.RuleCheckerError("Sensitivity can only be "
                                               "between 1 and 4.")
    ...
    def test_rule_file(self):
        ...
        try:
            ...
            raise RuleChecker.RuleCheckerError(
                "Error executing Dumbpig. Please try again.")

        dp_output = proc_out.stdout.read()
        self._dumbpig_output = dp_output

        # cleans up 0 file created by dumbpig
    ...
    def process_output(self):
        ...
        if not self._dumbpig_output:
            raise RuleChecker.RuleCheckerError("No data to process. Run "
                                               "the test then process results")
        # Parses the output data with regex to split up each Issue
        regex_issues = re.compile(r"Issue [1-9][0-9]* \n(.*?)\n\=", re.DOTALL)
        output_data = regex_issues.findall(self._dumbpig_output)

        regex_problem = re.compile(r"(.*?)\n\nalert")
        regex_fix = re.compile(r"\)\n(.*?)\n\n", re.DOTALL)
        regex_rule = re.compile(r"[0-9] \n(alert.*)")
        processed_data = {}
        for stuff in output_data:
            line = int(regex_problem.findall(stuff)[0].rsplit(" ", 3)[1])
            fix = regex_fix.findall(stuff)[0].replace(
                "\n", "").replace(" - ", "/").replace("   ", "").split("- ")
            fix.pop(0)
            rule = regex_rule.findall(stuff)[0]
            processed_data[line] = {"fix": fix, "rule": rule}
    ...
    def json_output(self):
        ...
        try:
            ...
            raise RuleChecker.RuleCheckerError("Error processing results in "
                                               "JSON format")
    ...
    class RuleCheckerError(Exception):
        ...
        def __init__(self, value):
            super(RuleChecker.RuleCheckerError, self).__init__(value)
            self.value = value
